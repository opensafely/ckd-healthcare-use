...
cap log close
macro drop hr
log using ./logs/relative_costs_dialysis.log, replace t
cap file close tablecontent
**Creates output file for each year (`dataset')
file open tablecontent using ./output/relative_costs_dialysis.csv, write text replace
**Column headings
...
qui su total_`cost'
local `x'_`cost' = (r(mean))/`denom'
}
file write tablecontent %9.2f (``x'_apcs_cost') _tab %9.2f (``x'_ec_cost') _tab %9.2f (``x'_opa_cost') _n
}
