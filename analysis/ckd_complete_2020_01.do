...
cap log close
log using ./logs/2020_01_ckd_complete, replace t
clear

*Merge 2020_01_ckd with 2020_01_ckd_complete
capture noisily import delimited ./output/2020_01_ckd.csv, clear
tempfile 2020_01_ckd
save 2020_01_ckd, replace
capture noisily import delimited ./output/input_2020_01_ckd_complete.csv, clear
merge 1:1 patient_id using 2020_01_ckd
keep if _merge==3
drop _merge
tempfile 2020_01_ckd_complete
save 2020_01_ckd_complete, replace
...
tab modality_baseline ukrr, m
egen ckd_group = cut(baseline_egfr), at(0, 30, 60, 5000)
recode ckd_group 0=3 30=2 60=1
replace ckd_group = 4 if ukrr=="Dialysis"
replace ckd_group = 5 if ukrr=="Transplant"
replace ckd_group = 1 if ckd_group==.
label define ckd_group 1 "Albuminuria" 2 "CKD stage 3" 3 "CKD stage 4/5" 4 "Dialysis" 5 "Transplant" 
label values ckd_group ckd_group
label var ckd_group "CKD group"
tab ckd_group, m
drop if ckd_group==.

*Incident KRT outcomes
rename ukrr_incident ukrr_string
gen ukrr_incident = 0
replace ukrr_incident = 1 if ukrr_string=="HD"
replace ukrr_incident = 1 if ukrr_string=="HHD"
replace ukrr_incident = 1 if ukrr_string=="ICHD"
replace ukrr_incident = 1 if ukrr_string=="PD"
replace ukrr_incident = 2 if ukrr_string=="Tx"
drop ukrr_string
label define ukrr_incident 0 "No KRT" 1 "Dialysis" 2 "Transplant"
label values ukrr_incident ukrr_incident
tab ukrr_incident, m
...
replace ckd_progression = 3 if ukrr_incident==1
replace ckd_progression = 4 if ukrr_incident==2
replace ckd_progression = 5 if died==1
label define ckd_progression 0 "No progression" 1 "CKD 3" 2 "CKD 4/5 pre-KRT" 3 "Dialysis" 4 "Kidney transplant" 5 "Deceased"
...
save "./output/2020_01_ckd_complete.dta", replace
log close
