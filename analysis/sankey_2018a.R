...
edges <- read_csv("edges_2018a.csv")
library("networkD3")
nodes_d3 <- mutate(nodes, id = id - 1)
edges_d3 <- mutate(edges, from = from - 1, to = to - 1)
# progression_2018a = object which can then be saved
progression_2018a <- sankeyNetwork(
  Links = edges_d3, Nodes = nodes_d3, 
  Source = "from", Target = "to", 
  NodeID = "label", Value = "weight", 
  fontSize = 36, unit = "Letter(s)")
# Save as HTML file
saveNetwork(progression_2018a, "2018a.html")
# Export from HTML to PNG
webshot::webshot("2018a.html","2018a.png", vwidth = 1000, vheight = 900)