---
title: "Absolute healthcare use (Kidney transplant patients)"
output: html_document
output_dir: "C:/Users/lsh1703468/Documents/Repo/ckd-healthcare-use/results"
output_file: "absolute_healthcare_use_transplant.html"
---
...
# Read in data
d <- read.csv("absolute_healthcare_use_transplant.csv", sep = "\t", na.strings = "N/A")
...
d1 <- rbind(
  ...
  d %>% mutate(count = avf, outcome = "AVF formations"),
  d %>% mutate(count = pd, outcome = "PD insertions"),
  d %>% mutate(count = opd, outcome = "Outpatient appointments"),
  d %>% mutate(count = neph, outcome = "Nephrology appointments"),
  d %>% mutate(count = tx, outcome = "Transplant appointments"),
...
plot_ymax = max(d2_tx$count, na.rm=T)*1.2

# Generate plot
ggplot(d2_tx, aes(x = date, y = count)) + 
  geom_line(size = 0.8) + 
  ylab("Transplant appointments") + 
  xlab("Year") +
...
plot_ymax = max(d2_avf$count, na.rm=T)*1.2

# Generate plot
ggplot(d2_avf, aes(x = date, y = count)) + 
  geom_line(size = 0.8) + 
  ylab("AVF formations") + 
  xlab("Year") +
...
plot_ymax = max(d2_pd$count, na.rm=T)*1.2

# Generate plot
ggplot(d2_pd, aes(x = date, y = count)) + 
  geom_line(size = 0.8) + 
  ylab("PD insertions") + 
  xlab("Year") +
