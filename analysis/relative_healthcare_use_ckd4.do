...
cap log close
macro drop hr
log using ./logs/relative_healthcare_use_ckd4.log, replace t
cap file close tablecontent
**Creates output file for each year (`dataset')
file open tablecontent using ./output/relative_healthcare_use_ckd4.csv, write text replace
**Column headings
...
local `x'_`aggregate' = (r(mean)/`denom')*1000
}
foreach binary of varlist fistula_formation pd_insertion blood_pressure albuminuria creatinine {
qui safecount if `binary'==1
local `x'_`binary' = (round(r(N),5)/`denom')*1000
}
file write tablecontent %4.2f (``x'_hospital_days') _tab %4.2f (``x'_critical_care_days') _tab %4.2f (``x'_emergency_days') _tab %4.2f (``x'_fistula_formation') _tab %4.2f (``x'_pd_insertion') _tab %4.2f (``x'_op_appts') _tab %4.2f (``x'_neph_appts') _tab %4.2f (``x'_tx_appts') _tab %4.2f (``x'_gp_interactions') _tab %4.2f (``x'_blood_pressure') _tab %4.2f (``x'_albuminuria') _tab %4.2f (``x'_creatinine') _n
}
